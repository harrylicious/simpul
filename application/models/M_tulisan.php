...
class M_tulisan extends CI_Model{
	...
	function get_berita_home(){
		$hsl=$this->db->query("SELECT tbl_tulisan.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_tulisan WHERE tulisan_kategori_nama<>'Sambutan' ORDER BY tulisan_id DESC limit 4");
		return $hsl;
	}
	
	function get_sambutan_home(){
		$hsl=$this->db->query("SELECT *FROM tbl_tulisan WHERE tulisan_kategori_nama='Sambutan'");
		return $hsl;
	}

	function berita_perpage($offset,$limit){
		$hsl=$this->db->query("SELECT tbl_tulisan.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_tulisan  WHERE tulisan_kategori_nama<>'Sambutan' ORDER BY tulisan_id DESC limit $offset,$limit");
		return $hsl;
	}

	function berita(){
		$hsl=$this->db->query("SELECT tbl_tulisan.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_tulisan  WHERE tulisan_kategori_nama<>'Sambutan' ORDER BY tulisan_id DESC");
		return $hsl;
	}
	...
	/* function populer(){
		$x =	$this->db->select('*','DESC')
				 ->from('tbl_tulisan')
				 ->order_by('tulisan_views')
				 ->limit(5)
				 ->get();
